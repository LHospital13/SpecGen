...
def spec_mutator(line):
    res_list = []
    if line.find("\\forall") != -1:
        res_list.append(line.replace("forall", "exists"))
    if line.find("\\exists") != -1:
        res_list.append(line.replace("exists", "forall"))
    if line.find("==") != -1:
...
def main():
    ...
    for i in range(1, args.max_iterations+1):
        ...
        if err_info == "" or len(err_types) == 0:
            break
    ...
    mutated_spec_list = []
    mutated_counter = 0
    while mutated_counter <= 32:
        for lineno in range(len(current_code_list)):
            if current_code_list[lineno].find("//@") != -1 and (current_code_list[lineno].find("ensures") != -1 or current_code_list[lineno].find("maintaining") != -1 or current_code_list[lineno].find("loop_invariant") != -1):
                for mutated_spec in spec_mutator(current_code_list[lineno]):
                    mutated_spec_list.append({"content": mutated_spec, "lineno": lineno})
                    mutated_counter = mutated_counter + 1
