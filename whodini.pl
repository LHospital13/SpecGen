...
my $openjml_executable = getcwd;
$openjml_executable = $openjml_executable."/openjml/openjml --esc";
...
while (1) {
    ...
    # 4 Merge the temp text file into the temp source files
    my $merge_script_path = getcwd;
    $merge_cmd = "perl ".$merge_script_path."/merge-esc.pl";
    print STDERR `cd $tmpdir && $merge_cmd -s $txtesctmp`;
    ...
    my @escoutput = `cd $tmpdir && $openjml_executable $notdir_sourcetmp`;
